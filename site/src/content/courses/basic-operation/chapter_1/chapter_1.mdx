...
> When you first connect your wallet, the 10 Live Cells assigned to you will have a 10-second delay. Please click the `Fetch Cells` button to refresh your cell list.
...
<br /># Testnet Chain It is also important to know the configuration information of this test chain.
...
`SCRIPTS` represent the built-in smart contracts, i.e. some of the system's built-in locks that type and lock scripts can use.

Several built-in smart contracts was pre-deployed in CKB testnet's genesis block. The following are three of the built-in smart contracts.
...
Well, that's all the information we need to know now.
