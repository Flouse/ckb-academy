...
In the CKB system, we compose the layer 2 network.
...
<Quote>
  ...
  Despite low throughput, only 1 transaction per month, you and your Amazon jungle buddy have created a layer2 network
  containing 2 nodes.
</Quote>



CKB offers tools for automatically building transactions, transferring contracts, etc. Nevertheless, we will continue with this old-school method to implement a regular transfer transaction.

By using manually assembled transactions, you will gain a deeper understanding of how CKB cells work.
...
Besides inputs, there is a field called cell_deps that indicates the dependency cells a transaction needs to rely on. This field appears as an index result in out_point.

#### What are dependency cells?
...
The output must have less capacity than the input, and the gap is the fee to reward miners.
...
Complete transactions also contain fields such as version and header_deps. The version field indicates the version, which is currently set to 0x0, while the header_deps field is null.
