...
import { ShowCells } from "~/content/courses/basic-operation/chapter_1/ShowCells.tsx";
import { ShowBlocks } from "~/content/courses/basic-operation/chapter_1/ShowBlocks.tsx";
...
When we say that a wallet has a certain amount of CKB (the native token), we are actually referring to the sum of the capacity of all the live cells that the wallet can unlock. This is also the total storage space that the wallet occupies on the chain.
